...
Deploys to a preview deployment if the \`${import_utils.CONVEX_DEPLOY_KEY_ENV_VAR_NAME}\` environment variable is set to a Preview Deploy Key.`
  ...
  if (deploymentSelection.kind === "preview") {
    ...
    if (cmdOptions.previewCreate !== void 0) {
      const source = deploymentSelection.kind === "deploymentWithinProject" && deploymentSelection.targetProject.kind === "deploymentName" ? `at ${import_chalk.chalkStderr.blue.underline(`https://dashboard.convex.dev/dp/${deploymentSelection.targetProject.deploymentName}/settings#preview-deploy-keys`)}` : deploymentSelection.kind === "existingDeployment" && deploymentSelection.deploymentToActOn.deploymentFields !== null ? `at ${import_chalk.chalkStderr.blue.underline(`https://dashboard.convex.dev/dp/${deploymentSelection.deploymentToActOn.deploymentFields.deploymentName}/settings#preview-deploy-keys`)}` : "on the dashboard";
      await ctx.crash({
        exitCode: 1,
        errorType: "fatal",
...
async function deployToNewPreviewDeployment(ctx, deploymentSelection, options) {
  ...
  const deploymentNameForWorkOS = previewUrl.match(/https:\/\/([^.]+)\.convex\.cloud/)?.[1] ?? null;
...
Your ${import_chalk.chalkStderr.bold(deployment.requestedType)} deployment ${import_chalk.chalkStderr.bold(
  ...
  ${(await (0, import_envvars.suggestedEnvVarName)(ctx)).envVar}=${import_chalk.chalkStderr.bold(prodUrl)}
